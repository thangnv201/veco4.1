...
#empty controllers action
    ...
    def get_all_project_users()
        ...
        res = {}
        current_member = Member.where(:project_id => @project.id, :user_id => User.current.id).first
        current_member_role = current_member.roles.map(&:name)
        user_list_ids = []
        role_config = get_roles_config();
        role_config.each do |role_cf|
            if current_member_role.include? role_cf["role_name"]
                role_list_lower = role_cf["lower_role"]
                member_list = Member.joins(:roles)
                                  .where("#{Role.table_name}.name" => role_list_lower)
                                  .where(:project_id => @project.id)
                user_list_ids += member_list.uniq.map(&:user_id)
            end
        end
