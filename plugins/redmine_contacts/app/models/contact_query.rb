...
class ContactQuery < Query
  ...
  def sql_for_has_open_issues_field(field, operator, value)
    value = value.map{ |val| val == 'me' ? User.current.id : val }

    db_table = ContactNote.table_name
    joins = "
      INNER JOIN #{CustomField.table_name} ON #{CustomField.table_name}.id = #{CustomValue.table_name}.custom_field_id
      INNER JOIN #{Issue.table_name} ON #{Issue.table_name}.id = #{CustomValue.table_name}.customized_id
      LEFT JOIN #{IssueStatus.table_name} ON #{IssueStatus.table_name}.id = #{Issue.table_name}.status_id
    "
    conditions = "
      (#{IssueStatus.table_name}.is_closed = #{ActiveRecord::Base.connection.quoted_false} OR #{IssueStatus.table_name}.is_closed IS NULL)
      AND #{CustomField.table_name}.type = 'IssueCustomField'
      AND #{CustomField.table_name}.field_format = 'contact'
    "

    if ["!*", "*"].include?(operator)
      contact_ids = CustomValue.joins(joins).where(conditions).pluck(:value).map(&:to_i).uniq
    else
      conditions += " AND #{sql_for_field("assigned_to_id", operator, value, Issue.table_name, 'assigned_to_id')}"

      contact_ids = CustomValue.joins(joins).where(conditions).pluck(:value).map(&:to_i).uniq
    end

    contact_ids.any? ? "#{Contact.table_name}.id IN (#{contact_ids.join(',')})" : "1=0"
  end
