...
class RedmineContacts::TestCase
  ...
  def self.prepare
    ...
    contacts_cf.name = 'Test related contacts'
    contacts_cf.field_format = 'contact'
    contacts_cf.is_for_all = true
    contacts_cf.multiple = true

    contacts_cf.tracker_ids = Tracker.pluck(:id)
    contacts_cf.save!

    [[1, 1], [1, 3], [2, 2]].each do |issue_id, contact_id|
      cv = contacts_cf.custom_values.build
      cv.customized = Issue.find(issue_id)
      cv.value = contact_id
      cv.save
    end

    # Create related deals
    deal_cf = CustomField.new_subclass_instance('IssueCustomField')
    deal_cf.name = 'Test related deals'
    deal_cf.field_format = 'deal'
    deal_cf.is_for_all = true
    deal_cf.multiple = true

    deal_cf.tracker_ids = Tracker.pluck(:id)
    deal_cf.save!

    [[1, 1], [2, 2], [3, 2]].each do |issue_id, deal_id|
      cv = deal_cf.custom_values.build
      cv.customized = Issue.find(issue_id)
      cv.value = deal_id
      cv.save
    end
