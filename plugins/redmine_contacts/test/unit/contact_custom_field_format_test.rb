...
class ContactCustomFieldFormatTest < ActiveSupport::TestCase
  ...
  RedmineContacts::TestCase.create_fixtures(
    ...
    [:contacts, :contacts_projects, :deals, :notes, :tags, :taggings, :queries]
  )

  def setup
    RedmineContacts::TestCase.prepare
    @field = IssueCustomField.create!(name: 'Tester', field_format: 'contact')
  end
  def test_possible_values_options_with_no_arguments
    with_contacts_settings('cross_project_contacts' => 0) do
      assert @field.possible_values_options.empty?
      assert @field.possible_values_options(nil).empty?
    end
  end
