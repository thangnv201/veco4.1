...
class ProductsController < ApplicationController
  before_action :find_product_project, :only => [:create, :new]
  before_action :find_product, :only => [:edit, :show, :destroy, :update, :add]
  before_action :bulk_find_products, :only => [:bulk_update, :bulk_edit, :bulk_destroy, :context_menu]
  ...
  helper :notes
  helper :queries
  helper :invoices if defined?(InvoicesHelper)
  include SortHelper
  ...
  def index
    ...
    if @query.valid?
      ...
      respond_to do |format|
        ...
        format.api
        format.csv { send_data(products_to_csv(@products), :type => 'text/csv; header=present', :filename => 'products.csv') }
      end
    else
      respond_to do |format|
  ...
  def show
    @recent_orders = Order.by_project(@project).joins(:lines).where(:product_lines => { :product_id => @product }).
                                                              visible.
                                                              limit(5).order("#{Order.table_name}.order_date DESC")
  end
  ...
  def bulk_update
    ...
    @products.each do |product|
      ...
      unsaved_product_ids << product.id unless product.save
    end
  ...
  def parse_params_for_bulk_product_attributes(params)
    attributes = (params[:product] || {}).reject { |_k, v| v.blank? }
