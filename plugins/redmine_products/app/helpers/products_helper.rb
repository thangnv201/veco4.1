...
module ProductsHelper
  ...
  def products_to_csv(products)
    return '' unless User.current.allowed_to?(:export_products, @project, :global => true)
    encoding = 'utf-8'
    export = FCSV.generate(:col_sep => l(:general_csv_separator)) do |csv|
      # csv header fields
      headers = ['#',
                 l(:label_products_code, :locale => :en),
                 l(:label_products_name, :locale => :en),
                 l(:label_products_description, :locale => :en),
                 l(:label_products_price, :locale => :en),
                 l(:label_products_category, :locale => :en),
                 l(:field_project, :locale => :en),
                 l(:field_author, :locale => :en),
                 l(:label_products_status, :locale => :en),
                 l(:label_products_currency, :locale => :en),
                 l(:field_created_on, :locale => :en),
                 l(:field_updated_on, :locale => :en)]

      custom_fields = ProductCustomField.order(:name)
      ...
      products.each do |product|
        ...
        product.custom_field_values.sort_by { |v| v.custom_field.name }.each do |custom_value|
          fields << RedmineContacts::CSVUtils.csv_custom_value(custom_value)
        end
  ...
  def importer_link
    project_product_imports_path
  end

  def importer_show_link(importer, project)
    project_product_import_path(:id => importer, :project_id => project)
  end

  def importer_settings_link(importer, project)
    settings_project_product_import_path(:id => importer, :project => project)
  end

  def importer_run_link(importer, project)
    run_project_product_import_path(:id => importer, :project_id => project, :format => 'js')
  end

  def importer_link_to_object(product)
    link_to product.name, edit_product_path(product)
  end

  def _project_products_path(project, *args)
    if project
      project_products_path(project, *args)
    else
      products_path(*args)
    end
  end

  def _project_orders_path(project, *args)
    if project
      project_orders_path(project, *args)
    else
      orders_path(*args)
    end
  end
