...
class Product < ActiveRecord::Base
  ...
  if ActiveRecord::VERSION::MAJOR >= 4
    has_one :image, lambda { where("#{Attachment.table_name}.description = 'default_image'") }, :class_name => 'Attachment',
                                                                                                :as => :container,
                                                                                                :dependent => :destroy
  else
    has_one :image, :conditions => "#{Attachment.table_name}.description = 'default_image'", :class_name => 'Attachment',
                                                                                             :as => :container,
                                                                                             :dependent => :destroy
  end
  ...
  acts_as_event :datetime => :created_at,
                ...
                :type => 'icon icon-product',
                :title => Proc.new { |o| "#{l(:label_products_product_created)} #{o.name} - #{o.price}" },
  ...
  validates_uniqueness_of :code, :allow_nil => true, :if => lambda { |product| product.code.present? }
  ...
  def self.last_code
    where("#{Product.table_name}.code IS NOT ?", nil).last.try(:code)
  end
  ...
  def visible?(usr = nil)
    (usr || User.current).allowed_to?(:view_products, project)
  end
