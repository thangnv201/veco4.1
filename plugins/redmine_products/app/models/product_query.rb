...
class ProductQuery < Query
  ...
  self.view_permission = :view_products if Redmine::VERSION.to_s >= '3.4' || RedmineContacts.unstable_branch?
  ...
  def initialize_available_filters
    add_available_filter 'ids', type: :integer, label: :label_product_plural if Redmine::VERSION.to_s >= '3.3'
    add_available_filter('status_id', type: :list_status, label: :label_products_status, order: 0,
                         values: collection_product_statuses.map { |k, v| [k, v.to_s] })

    add_available_filter 'code', type: :string, label: :label_products_code
    add_available_filter 'name', type: :string, label: :label_products_name
    add_available_filter 'price', type: :float, label: :label_products_price
    ...
    add_available_filter('category_id', type: :list, label: :label_products_category, values: product_categories) if product_categories.any?
