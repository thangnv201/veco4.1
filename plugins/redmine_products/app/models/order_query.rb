...
class OrderQuery < Query
  ...
  self.view_permission = :view_orders if Redmine::VERSION.to_s >= '3.4' || RedmineContacts.unstable_branch?
  ...
  def initialize_available_filters
    ...
    add_available_filter 'ids', :type => :integer, :label => :label_order_plural if Redmine::VERSION.to_s >= '3.3'
  ...
  def sql_for_contact_field(field, operator, value)
    sql_for_field(field, operator, value, Order.table_name, 'contact_id')
  end
  ...
  def query_includes
    ...
    includes << { :contact => :address } if self.filters['contact_country'] || filters['contact_city'] ||
