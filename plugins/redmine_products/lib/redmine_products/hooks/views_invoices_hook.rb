  ...
  module Hooks
    class ViewsInvoicesHook < Redmine::Hook::ViewListener
      ...
      def edit_invoices_form_details_bottom(context = {})
        javascript_tag "observeProductAutocompleteField('new_product_code', '#{escape_javascript auto_complete_products_path }', '#{escape_javascript add_products_path(:line_class => :invoice)}')"
      end
      ...
      def view_invoices_show_lines_bottom(context = {})
        if order = context[:invoice] && context[:invoice].order_number && Order.visible.where(:number => context[:invoice].order_number).first
          ...
          javascript_tag "$('td.oder-number').html('#{escape_javascript order_link}');"
        end
      end
    end
  end
