...
Rails.configuration.to_prepare do
  ...
  if Redmine::VERSION.to_s < '2.5'
    require 'redmine_products/patches/custom_field_format_patch'
  else
    require 'redmine_products/patches/field_format_patch'
  end
  ...
  require 'redmine_products/patches/invoice_line_patch'
  require 'redmine_products/patches/invoice_line_drop_patch'

  require 'redmine_products/hooks/views_layouts_hook'
  require 'redmine_products/hooks/controller_contacts_duplicates_hook'

  require 'redmine_products/liquid/liquid' if Object.const_defined?("Liquid") rescue false
  require 'redmine_products/compatibility/application_helper_patch'
end
...
module ProductsSettings
  ...
  def self.select_companies
    Setting.plugin_redmine_products["select_companies"].to_i > 0
  end
  ...
  def self.invoices_fully_compatible?
    Redmine::Plugin.find(:redmine_contacts_invoices).version >= '4.1.0'
  end
end
