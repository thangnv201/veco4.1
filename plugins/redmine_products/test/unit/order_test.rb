...
class OrderTest < ActiveSupport::TestCase
  ...
  def test_order_completed_date
    order = Order.find(1)
    completed_status = OrderStatus.completed.first
    assert_nil order.completed_date

    order.update_attributes(:status => completed_status)
    order.reload
    assert_not_equal order.completed_date, nil
    assert_equal order.status, completed_status

    order.update_attributes(:description => 'Test test test')
    order.reload
    assert_not_equal order.completed_date, nil
    assert_equal order.description, 'Test test test'
    assert_equal order.status, completed_status
  end
