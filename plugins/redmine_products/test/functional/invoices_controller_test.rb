...
class InvoicesControllerTest < ActionController::TestCase
  ...
  def test_get_show_with_related_product
    ...
    invoice_line = invoice.lines.last
    product = Product.first
    invoice_line.update_attributes(:product_id => product.id)

    compatible_request :get, :show, :id => invoice.id
    assert_response :success
    assert_select 'td.description a', 1
  end if ProductsSettings.invoices_plugin_installed?
  ...
  def test_get_edit_with_related_product
    ...
    compatible_request :get, :edit, :id => invoice.id
    assert_response :success
    assert_select 'td.product-image a.avatar', 1
  end if ProductsSettings.invoices_plugin_installed?
end
