...
class Redmine::ApiTest::OrdersTest < ActiveRecord::VERSION::MAJOR >= 4 ? Redmine::ApiTest::Base : ActionController::IntegrationTest
  ...
  def test_get_orders_xml
    ...
    compatible_api_request :get, '/orders.xml', {}, credentials('admin')

    assert_select 'orders', :attributes => { :type => 'array',
  ...
  def test_post_orders_xml
    parameters = { :order => { :number => 'api_test_002',
    ...
    if ActiveRecord::VERSION::MAJOR < 4
      Redmine::ApiTest::Base.should_allow_api_authentication(:post, '/orders.xml', parameters, :success_code => :created)
    end

    assert_difference('Order.count') do
      compatible_api_request :post, '/orders.xml', parameters, credentials('admin')
    end
    ...
    assert_select 'order', :child => { :tag => 'id', :content => order.id.to_s }
  end
  ...
  def test_put_orders_1_xml
    ...
    if ActiveRecord::VERSION::MAJOR < 4
      Redmine::ApiTest::Base.should_allow_api_authentication(:put, '/orders/1.xml', parameters, :success_code => :ok)
    end

    assert_no_difference('Order.count') do
      compatible_api_request :put, '/orders/1.xml', parameters, credentials('admin')
    end
  ...
  def test_should_put_with_custom_fields
    ...
    assert_no_difference('Order.count') do
      compatible_api_request :put, '/orders/1.xml', { :order => { :custom_fields => [{ 'id' => field.id.to_s, 'value' => 'Hello' }] } }, credentials('admin')
    end
    order = Order.find(1)
