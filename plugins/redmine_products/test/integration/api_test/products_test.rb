...
class Redmine::ApiTest::ProductsTest < ActiveRecord::VERSION::MAJOR >= 4 ? Redmine::ApiTest::Base : ActionController::IntegrationTest
  ...
  def test_update_product_with_uploaded_file
    ...
    assert_difference 'Attachment.count' do
      compatible_api_request :post, '/uploads.xml', 'test_upload_with_upload',
    ...
    compatible_api_request :put, '/products/1.xml', { :product => { :name => 'Attachment added',
                                                                    :uploads => [{ :token => token,
                                                                                   :filename => 'test.txt',
  ...
  def test_should_put_with_custom_fields
    ...
    assert_no_difference('Product.count') do
      compatible_api_request :put, '/products/1.xml', { :product => { :custom_fields => [{ 'id' => field.id.to_s, 'value' => 'Hello' }] } }, credentials('admin')
    end
    product = Product.find(1)
