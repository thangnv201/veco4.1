...
class Person < User
  ...
  has_many :work_experiences, class_name: 'PeopleWorkExperience', foreign_key: :user_id, dependent: :destroy
  ...
  def all_visible_work_experiences
    work_experiences.order(:from_date => :desc) if visible?
  end

  def workday_length
    information.try(:workday_length) || Setting.plugin_redmine_people['workday_length'].to_f
  end

  def first_activity_date
    time_entries.order(:spent_on).first.try(:spent_on)
  end
