...
class PeopleController < ApplicationController
  ...
  def bulk_edit
    render_403 unless bulk_edit_access?
  end

  def bulk_update
    return render_403 unless bulk_edit_access?
    unsaved_people_ids = []
    @people.each do |person|
      person.reload
      params[:person][:tag_list] = (person.tag_list + RedmineCrm::TagList.from(params[:add_tag_list]) - RedmineCrm::TagList.from(params[:delete_tag_list])).uniq

      unless person.update_attributes(parse_params_for_bulk_people_attributes(params.respond_to?(:to_unsafe_hash) ? params.to_unsafe_hash.symbolize_keys : params))
        # Keep unsaved issue ids to display them in flash error
        unsaved_people_ids << person.id
      end
    end
    set_flash_from_bulk_people_save(@people, unsaved_people_ids)
    redirect_back_or_default({controller: 'people', action: 'index'})
  end

  def parse_params_for_bulk_people_attributes(params)
    attributes = (params[:person] || {}).reject { |k, v| k != 'tag_list' && v.blank? }
    attributes.each { |k, v| attributes[k] = v.reject { |_x, z| z.blank? } if v.is_a?(Hash) }
  ...
  def get_data_for_tab
    ...
    @prfm_collector = PersonPerformanceCollector.new(@person, params[:interval_type], params[:date_period]) if tab == 'performance'
  end
end
