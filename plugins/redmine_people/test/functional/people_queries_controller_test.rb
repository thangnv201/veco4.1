...
class PeopleQueriesControllerTest < ActionController::TestCase
  ...
  def test_post_create_people_public_query_without_permission
    ...
    compatible_request :post, :create, :default_columns => '1',
                                             :f => ['firstname', 'lastname'],
                                             :op => { 'firstname' => '=', 'lastname' => '=' },
                                             :v => { 'firstname' => ['Ivan'], 'lastname' => ['Ivanov'] },
                                             :query => query_params

    q = PeopleQuery.where(:name => 'test_new_public_people_query').first
  ...
  def test_put_update_public_query
    ...
    q = PeopleQuery.where(:name => 'test_edit_public_query').first
    assert_redirected_to :controller => 'people' , :action => 'index', :query_id => q.id
    assert q.is_public?
    assert q.has_default_columns?
    assert q.valid?
  ...
  def test_delete_destroy
    ...
    assert_redirected_to :controller => 'people', :action => 'index', :set_filter => '1'
    assert_nil Query.where(:id => 2).first
  end
end
