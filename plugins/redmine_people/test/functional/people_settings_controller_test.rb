...
class PeopleSettingsControllerTest < ActionController::TestCase
  ...
  RedminePeople::TestCase.create_fixtures(Redmine::Plugin.find(:redmine_people).directory + '/test/fixtures/', [:leave_types])
  ...
  def test_announcements_tab
    with_people_settings 'use_announcements' => '1' do
      compatible_request :get, :index
      assert_response :success
      assert_select '#tab-people_announcements'
      assert_select '#tab-content-people_announcements'
    end
  end

  def test_get_index_with_leave_types_tab
    compatible_request :get, :index
    assert_response :success
    assert_select '#tab-leave_types'
    assert_select '#tab-content-leave_types table tbody tr', LeaveType.all.size
  end
end
